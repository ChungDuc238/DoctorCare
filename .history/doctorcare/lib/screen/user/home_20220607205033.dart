...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget upCommingAppointment(BuildContext context) {
    final Stream<QuerySnapshot> listAppoinment =
        FirebaseFirestore.instance.collection('chuyenmon').snapshots();
        
    return Column(
      children: [
        const Text('Danh mục bác sĩ theo chuyên môn'),
        StreamBuilder<QuerySnapshot>(stream: listAppoinment, builder: (BuildContext conntext, AsyncSnapshot<QuerySnapshot> snapshot){
          if(snapshot.hasError){
            return Text('Lỗi khi lấy dữ liệu')
          }
        },)
      ],
    );
