...
import '../doctor/doctor_appointment_screen.dart';
                            ...
                            children: [
                              ...
                              GridView.builder(
                                ...
                                itemBuilder: ((context, i) {
                                  String key = data.docs[index]['thoigian'].keys
                                      .elementAt(i);
                                  trangthai = data.docs[index]['thoigian'][key];
                                  var time1 = key.split('-');
                                  var time2 = time1[0].split(':');
                                  int checkBuoi =
                                      convertStringToInt(time2[0]) ?? 0;
                                  if (checkBuoi >= 7 && checkBuoi <= 12) {
                                    return InkWell(
                                      child: trangthai.compareTo('false') == 0
                                          ? DaCoNguoiDat(data: data)
                                          : InkWell(
                                              onTap: () {
          ...
          cb_press: () async {
            ...
            try {
              ...
              final DocumentReference referenceLichKham =
                  FirebaseFirestore.instance.collection('lichkham').doc(idLK);
...
class DaCoNguoiDat extends StatelessWidget {
  const DaCoNguoiDat({
    Key? key,
    required this.data,
  }) : super(key: key);
