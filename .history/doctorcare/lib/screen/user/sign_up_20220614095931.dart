...
class _SignUpScreenState extends State<SignUpScreen> {
                    ...
                    BtnPrimary(
                      ...
                      cb_press: () => {
                        _signUp(
                            ...
                            _role.toString().split('.').last.toLowerCase())
                      },
                    ),
                    const SizedBox(
                      height: 10,
  ...
  void _signUp(
    ...
    await authRepository.putEmailToFireStore(role, password, firstName);
    var id = await FirebaseAuth.instance.currentUser?.uid;
    // await authRepository.putID();
    if (result && role.compareTo('user') == 0) {
      // ignore: use_build_context_synchronously
      Navigator.push(
          context, MaterialPageRoute(builder: (context) =>  HomeScreen(id: id!)));
    }
    if (result && role.compareTo('doctor') == 0) {
      // ignore: use_build_context_synchronously
      Navigator.push(context,
          MaterialPageRoute(builder: (context) =>  HomeDoctorScreen(id: id!,)));
    }
  }
}
