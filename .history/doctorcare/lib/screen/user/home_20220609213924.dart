...
//
  Widget listDoctor(BuildContext context) {
    ...
    final doc = FirebaseFirestore.instance.collection('BacSi').doc();
    final String listid = doc.id[1];
    return SizedBox(
      width: 400,
                    ...
                    child: Container(
                      ...
                      child: SingleChildScrollView(
                        child: _doctorItem(
                            context,
                            '${data.docs[index]['ten']}',
                            '${data.docs[index]['benhvien']}',
                            '${data.docs[index]['chuyenmon']}',
                            '${FirebaseFirestore.instance.collection('BacSi').doc().id[index]}'),
                      ),
                    ),
                  );
                });
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => DoctorInfo(user.id ?? '')));
      },
