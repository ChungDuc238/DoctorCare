...
class DoctorAppointmentScreenState extends State<DoctorAppointmentScreen> {
            ...
            Row(
              children: [
                InkWell(
                  ...
                  onTap: () async {
                    Map<String, String> kq = autoGenerate();
                    if (list.isEmpty) {
                      list.addAll(kq);
                      log(list.toString());
                      setState(() {});
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: ((context, index) {
                        return DoctorTimingData(
                            time: list.keys.elementAt(index),
                            isSelected: false);
                      }),
                      gridDelegate:
          ...
          onPressed: () async {
            if (list.isEmpty) {
              const snackBar = SnackBar(
                content: Text('Thêm lịch khám thất bại'),
              );
              ...
              const snackBar = SnackBar(
                content: Text('Thêm lịch khám thành công'),
              );
              ScaffoldMessenger.of(context).showSnackBar(snackBar);
            }
  ...
  Map<String, String> autoGenerate() {
    result = caKham(startTime, endTime);
    var time1 = result.split('-')[0];
    var time2 = result.split('-')[1];
    var minute1 = int.parse(time1.split(':')[1]);
    var minute2 = int.parse(time2.split(':')[1]);
    Map<String, String> kq = {};
    if (minute1 != 00 || minute2 != 00) {
      kq = {};
    } else {
      var hourStart = convertStringToInt(time1);
      var hourEnd = convertStringToInt(time2);
      if (hourEnd! < hourStart!) {
        // showTopSnackBar(
        //     context,
        //     const CustomSnackBar.error(
        //         message: 'giờ bắt đầu không được lớn hơn giờ kết thúc'));
        const snackBar = SnackBar(
          content: Text('Giờ bắt đầu không được lớn hơn giờ kết thúc'),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      } else {
        for (int i = hourStart; i < hourEnd; i++) {
          if (i == 12) {
            ...
            for (int j = 0; j <= 50; j = j + 10) {
              if (j == 50) {
                Map<String, String> list1 = {'$i:$j-${i + 1}:00': 'true'};
                kq.addAll(list1);
              } else {
                Map<String, String> list2 = {'$i:$j-$i:${j + 10}': 'true'};
                kq.addAll(list2);
              }
            }
          }
    ...
    // log(hourStart.toString());
    // log(hourEnd.toString());
  ...
  String caKham(TimeOfDay start, TimeOfDay end) {
    if (start.minute == 0) {
      if (end.minute == 0) {
        return '${start.hour}:00-${end.hour}:00';
      } else {
        return '${start.hour}:00-${end.hour}:${end.minute}';
      }
    } else {
      if (end.minute == 0) {
        return '${start.hour}:${start.minute}-${end.hour}:00';
      }
    }
  ...
  Widget _buildTimePick(
    ...
    if (minute == 0) {
      ...
      //     const CustomSnackBar.error(message: 'Bạn không được chọn giờ lẻ'));
      const snackBar = SnackBar(
        content: Text('Bạn không được chọn giờ lẻ'),
      );
      Future.delayed(Duration.zero,()Ơ)
    }
...
int? convertStringToInt(String s) {
  var hourStart = s.split(':');

  return int.tryParse(hourStart[0]);
}
